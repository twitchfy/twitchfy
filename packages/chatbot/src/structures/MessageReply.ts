...
export class MessageReply<T extends EventSubConnection> extends Base<T> {
  ...
  public constructor(chatbot: ChatBot<T>, data: NonNullable<MessageData<T>['message']['reply']>, chatroom: ChatRoom<T>){
    ...
    this.thread = new MessageReplyThread(chatbot, { id: data.thread.messageId, user_id: data.thread.user.id, user_name: data.thread.user.displayName, user_login: data.thread.user.login, chatroom_id: chatroom.id }, chatroom);
  }
}