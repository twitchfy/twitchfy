...
export function parseTags(message: string) {
  ...
  const dictParsedTags = {};  // Holds the parsed list of tags.
  ...
  parsedTags.forEach(tag => {
    ...
    case 'badge-info':
      ...
      if (tagValue) {
        const dict = {};  // Holds the list of badge objects.
    ...
    case 'emotes':
      ...
      if (tagValue) {
        const dictEmotes = {};  // Holds a list of emote objects.
        ...
        emotes.forEach(emote => {
          ...
          positions.forEach(position => {
            const positionParts = position.split('-') as string[];
            textPositions.push({
